...
const NavbarTtem=({title, classProps})=>{
    return(
    ...
    return (
        <nav className='w-full flex md:justify-center justify-between items-center p-4'>
            ...
            {["Market","Exchange","Tutorial","Wallet"].map((item, index)=>(
                    <NavbarTtem key={item+index} title={item}/>
            ...
            <div className='flex relative'>
                ...
                :<HiMenuAlt4 fontSize={28} className='text-white md:hidden cursor-pointer' onClick={()=>setToggleMenu(true)}/>}
                {
                    ToggleMenu && (
                        ...
                        list-none flex flex-col justify-start items-end rounded-md 
                        blue-glassmorphism text-white animate-slide-in'>
                            <li className='text-xl w-full my-2'>
                                <AiOutlineClose onClick={()=>setToggleMenu(false)}/>
                            </li>
                            {["Market","Exchange","Tutorial","Wallet"].map((item, index)=>(
                    <NavbarTtem key={item+index} title={item} classProps={"my-2 text-lg"}/>
